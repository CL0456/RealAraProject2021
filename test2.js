...
function ready() {
 ...
 document.addEventListener('DOMContentLoaded', function() {
	...
	//On load of page, listeners are set-up//
		"use strict"
		setUpListeners();
		ready();
 	})	
		...
		document.getElementById("add_button").addEventListener('click', function() {addDestination_onClick();});
		...
		document.getElementById("redo_button").addEventListener('click',function(){removeItem_onClick();})
		document.getElementById("closePage").addEventListener('click',function(){closeOpenedWindow();})
		document.getElementById("distances_calc").addEventListener('click',function(){destdistCalc();})
	}
...
function addDestination_onClick() {
	...
	destinations.splice(destinations.length,0, destination)
	console.log(destinations)
}

//Removes previously added Location from list//
function removeItem_onClick() {
...
function destdistCalc() {
    ...
    let destList = destinations //empty array//
    let townList = towns //array with location names//
    let townDistance = distances // distances array//
        i = 0
        traveled = 0
        while((destList.length -1) > i) {
           location1 = townList.indexOf(destList[i])
        	location2 = townList.indexOf(destList[(i+1)])
            mileage = townDistance[Number(mileage1)]
            traveled += mileage[Number(mileage2)]
            i+=1
            console.log(traveled, mileage1, mileage2, mileage)
